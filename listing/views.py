...
class ManageListingView(APIView):
    def get(self, request, format=None):
        try:
            ...
            if not user.is_realtor:
                return Response(
                    {'error': 'User does not have neccessary permissions for getting this listing data'},
    ...
    def post(self, request):
        try:
            ...
            if not user.is_realtor:
                return Response(
                    {'error': 'User does ot have neecessary Permissions for creating this listing data'},
                    status=status.HTTP_403_FORBIDDEN
                )
            ...
            return Response(
                {'error': 'Somethting went wrong when creating listing'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
